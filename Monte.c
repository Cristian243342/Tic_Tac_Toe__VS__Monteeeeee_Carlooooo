...
// Returns -1 is 0 wins, 0 if it's a tie, 1 if X wins, or 2 if
...
mc_node_t *next_expansion(mc_node_t *node)
{
	...
	for (i = r / 3; count_i; i = (i + 1) % 3, count_i--)
		for (j = r % 3, count_j = 3; count_j; j = (j + 1) % 3, count_j--)
...
// Deallocates tree's memory from Monte Carlo algorithm
  ...
  // If the player is the first to start the game
	...
	while (game_is_over(tree_head->image) == 2) {
		...
		if (ver) {
			...
			// (Suggestion): put comments here put comments here put comments here
			// put comments here put comments here put comments here put comments here
